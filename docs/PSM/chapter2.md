...
The short-term rental market in Malaysia has grown over the past decade alongside domestic tourism. A 2023 study of the Malaysian homestay sector recorded 219 registered houses generating a cumulative income of RM29.6 million under the Inap Desa cluster programme as of 2019, before international arrivals dropped by 83% between 2019 and 2020 due to the COVID-19 pandemic (Zulkifli et al., 2023). The regulatory landscape for short-term rentals in Malaysia remains fragmented, with each state maintaining its own framework rather than a unified federal approach, which further reinforces the need for independent operators to adopt private management tools rather than relying solely on public marketplace platforms (Geoffrey, Sulaiman and Samad, 2024). Research into the role of ICTs in Malaysian homestay operations has further shown that digitalisation positively affects the livelihood assets of community-based operators across human, financial, physical, and social capital dimensions (Gan, Inversini and Rega, 2018). Many operators started with one property and added more over time. A single unit is manageable: bookings are infrequent enough to track without a formal system, payments follow a predictable pattern, and the whole operation fits within a single person's phone. The situation changes when a second or third unit is added. Five units running simultaneously requires five separate availability records to be maintained, five sets of payments to be monitored, and on a busy weekend, five different guests potentially checking in and out on the same day. That volume of coordination does not work when the only tool available is a mobile phone and a collection of chat threads.
...
### 2.6.10.0 Web-Based Booking System Design

Ng et al. (2022) demonstrated that a web-based booking platform can enable users to find venues, submit bookings, and track expenses within a single integrated system. Their event-venue booking and management system shares a comparable multi-module architecture with HomeLodge: both systems consolidate availability checking, booking submission, and payment tracking into one platform rather than distributing them across disconnected tools. The reference architecture described by Ng et al. provides a comparative baseline for evaluating HomeLodge's design decisions.
...
A Scopus-indexed comparative study by Laaziri et al. (2019) evaluated Laravel and Symfony, two of the most widely used PHP frameworks, across criteria including code structure, built-in tooling, community support, and development speed. Their analysis provides additional academic grounding for the selection of Laravel as HomeLodge's backend framework.
...
Murley et al. (2021) conducted an empirical study of WebSocket adoption across over 55,000 websites, establishing WebSocket as the dominant protocol for real-time web communication in production environments. Their findings confirm that WebSocket is no longer an emerging technology but a mature, widely deployed standard for applications that require persistent bidirectional connections.
...
Shin, Jung, Chang, and Lee (2012) examined the security properties of QR codes used in mobile authentication and access control. They found that a static QR code presents an unauthorised-copy risk: any person who obtains an image of a valid code can use it to gain access at any time. Their recommended mitigation is to encode a time-limited token, so that a copied code becomes invalid once the original session expires. More recently, Gupta et al. (2024) established that two-factor authentication using QR codes requires users to provide two forms of evidence, adding a verifiable security layer to access control systems.

HomeLodge applies this approach. The QR code for each booking encodes a unique token valid from the check-in time to the check-out time. The token is invalidated automatically at check-out and regenerated for the next confirmed booking only after housekeeping is marked complete. A copy of the guest's QR code cannot be used after the stay ends. This design directly addresses the vulnerability identified by Shin et al. (2012) and aligns with the two-factor authentication principle described by Gupta et al. (2024).
