...
All pages follow a consistent structure within their respective layouts. The table below summarises the key differences:

| Characteristic | Guest interface | Admin interface |
|---|---|---|
| Layout model | Top navbar, full-width content area (max 1200px, centred), footer | Fixed left sidebar, top header bar, fluid content area |
| Minimum supported screen width | 320px (mobile phones) | 768px (tablets); sidebar collapses to icons below this width |
| Grid system | 8px base grid for spacing and component alignment | 8px base grid for spacing and component alignment |
| Typography | Inter (Google Fonts), base size 16px, line height 1.5 | Inter (Google Fonts), base size 14px for tables and dense data views, 16px for body text |
...
- The guest navbar contains: Home, My Bookings, Notifications (a bell icon with an unread count badge), and a user avatar dropdown that leads to Profile and Logout.
- The admin sidebar groups items under labelled sections: Dashboard, Homestays, Bookings, Payments, QR Access, Guests, Users, Access Control (Roles, Permissions), Communication (Chat, Notifications), and System (Settings, Audit Logs).
...
Several design decisions were made specifically to reduce the effort required from the people who use this system on a daily basis:

- Form fields use appropriate input controls rather than free text wherever possible. Dates are selected from a date picker, times from a time picker, and statuses from dropdowns. This prevents format errors before they happen.
- Destructive actions always require a second confirmation step. When a user clicks Delete or Cancel, a modal dialog appears with a red-coloured confirm button. The user cannot accidentally confirm by pressing Enter alone; they must deliberately click the button.
- The booking calendar communicates availability through colour fills (green for available, red for booked, grey for blocked, amber for pending payment) so that an admin or guest can scan an entire month at a glance without reading individual date labels.
- Error messages appear directly below the input field that caused them. If the first error is off-screen, the page scrolls to it automatically. Each error message states what went wrong and what the user should do (e.g., "Password must be at least 8 characters. You need 2 more.") rather than showing a generic notice.
- Success and failure feedback appears as toast notifications at the top right corner. These auto-dismiss after 5 seconds so the user's workflow is not interrupted, but they remain long enough to be read.
- On the admin dashboard, summary cards load first while charts render in the background. The admin sees useful numbers immediately rather than waiting for the full page to finish loading.
- Long tables paginate at 10 rows by default. Column headers are sortable, and row-level actions are represented by icon buttons with tooltips. The admin does not have to scroll through hundreds of entries to find what they need.
- On mobile devices, all touch targets (buttons, links, checkboxes) are at least 44 by 44 pixels to reduce the chance of mis-taps.

**Usability requirements**

The table below lists specific, testable requirements that define how the system should behave from the user's perspective. These are written so that each one can be verified during testing.
...
| UI-USE-08 | Colour is never the only way to communicate information. Every status badge pairs its colour with a text label (e.g., a green badge that reads "Confirmed"), and every chart includes a legend or data labels. |
